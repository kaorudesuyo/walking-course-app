...
export default function CourseCard({ course, index }: { course: Course; index: number }) {
  return (
    <Link
      href={`/courses/${course.id}?data=${encodeURIComponent(JSON.stringify(course))}`}
      style={{ display: "block", textDecoration: "none" }}
      className={`anim-fade-up anim-delay-${Math.min(index + 1, 3)}`}
    >
      <div
        className="surface"
        style={{ padding: "28px 32px", cursor: "pointer", transition: "border-color 0.25s, background 0.25s" }}
      ...
      >
        ...
        <div style={{ display: "flex", gap: 28, marginBottom: 20, paddingBottom: 20, borderBottom: "1px solid var(--border)" }}>
          {[
            ...
            <div key={lbl}>
              <div style={{ fontFamily: "'Jost', sans-serif", fontSize: "1.1rem", fontWeight: 300, color: "var(--white)", letterSpacing: "0.02em" }}>
        ...
        <p style={{ fontFamily: "'Noto Serif JP', serif", fontSize: "0.82rem", color: "var(--muted)", lineHeight: 1.9, margin: 0, fontWeight: 300,
