...
export default function LocationPicker({ onLocationSet, location }: Props) {
  ...
  async function handleManualSearch(e: React.FormEvent) {
    ...
    try {
      ...
      const r = results[0];
      onLocationSet({ lat: parseFloat(r.lat), lng: parseFloat(r.lon), label: query });
      setShowInput(false);
  ...
  const spinnerStyle: React.CSSProperties = {
    width: 13, height: 13, border: "1px solid rgba(255,255,255,0.3)",
    borderTopColor: "currentColor", borderRadius: "50%",
    display: "inline-block", animation: "spin 0.9s linear infinite", flexShrink: 0,
  };
    ...
    return (
      <div>
        ...
        <div className="surface" style={{ padding: "14px 18px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "1.05rem", fontWeight: 300, color: "var(--white)", letterSpacing: "0.04em" }}>
              {location.label}
            </div>
            <div style={{ fontFamily: "'Jost', sans-serif", fontSize: "0.62rem", color: "var(--muted)", marginTop: 3, letterSpacing: "0.1em" }}>
              {location.lat.toFixed(5)}, {location.lng.toFixed(5)}
            </div>
          </div>
          <button className="btn btn-ghost" onClick={() => { onLocationSet(null!); setShowInput(false); }}>
            変更
          </button>
        </div>
      </div>
  ...
  return (
    <div>
      <span className="label" style={{ display: "block", marginBottom: 12 }}>Location</span>
      <button className="btn btn-white" style={{ width: "100%", marginBottom: 10 }}
        onClick={handleGeolocate} disabled={loading}>
      ...
      {!showInput ? (
        <button className="btn btn-outline" style={{ width: "100%" }} onClick={() => setShowInput(true)}>
          地名・住所で検索
        </button>
      ) : (
        <form onSubmit={handleManualSearch} style={{ display: "flex", gap: 8 }}>
          <input className="input" type="text" value={query} onChange={(e) => setQuery(e.target.value)}
            placeholder="例: 渋谷、上野公園" autoFocus />
          <button className="btn btn-white" style={{ padding: "12px 20px", flexShrink: 0 }}
            type="submit" disabled={geocoding || !query.trim()}>
            {geocoding ? <span style={spinnerStyle} /> : "検索"}
          </button>
        </form>
      )}
      {error && (
        <div style={{ marginTop: 10, fontFamily: "'Jost', sans-serif", fontSize: "0.68rem", color: "rgba(255,120,120,0.9)", letterSpacing: "0.06em" }}>{error}</div>
      )}
    </div>
  );
}
