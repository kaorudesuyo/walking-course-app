...
/**
 * セキュリティ/信頼性: 共有リンクには routeGeoJson を含めない。
 * OSRMの座標列は数十KB〜数百KBになりURLが肥大化し、
 * サーバーのURL長制限超過やDoSの温床になる。
 * 詳細ページはiframe地図を使うため routeGeoJson を必要としない。
 */
function buildShareData(course: Course): string {
  const { routeGeoJson: _omit, ...shareable } = course;
...
export default function CourseCard({ course, index }: { course: Course; index: number }) {
  return (
    <Link href={`/courses/${course.id}?data=${buildShareData(course)}`}
